...
declare module './state+main' {
  ...
  export interface Impl<T> extends Type<T> {
    _forEach(selector: ForEach<T>, ssPath: Try<string>, current: Nullable<number>): void;
...
Impl.prototype._createSingleBranches = function <T>(ssKey: Nullable<string>): [string, Type<T>][] {
  let substateBranches = Try.success(Objects.entries(this._substate))
    ...
    .map(v => v.map((v1): [string, Impl<T>][] => {
      let branches = v1[1]._createSingleBranches(v1[0]);

      if (branches.length > 0) {
        return branches.map((v2): [string, Impl<T>] => {
          return [v1[0], <Impl<T>>empty<T>().updatingSubstate(v2[0], v2[1])];
        });
      } else {
        return [v1];
      }
